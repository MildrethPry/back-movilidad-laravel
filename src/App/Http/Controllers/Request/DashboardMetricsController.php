...
class DashboardMetricsController extends Controller
{
    ...
    private function secretaria(User $user): array
    {
        ...
        $todayTrips = RouteSheet::whereHas('request', fn ($q) => $q->whereDate('departure_date', now()->toDateString()))->count();
        $inWorkshop = Vehicle::where('operational_status', 'en_taller')->count();
        $gallons = (float) FuelOrder::where('order_status', 'despachada')
            ->whereBetween('dispatch_date', [now()->startOfMonth(), now()->endOfMonth()])
        ...
        $recent = MobilizationRequest::with('requester')
            ->orderByDesc('id')
            ->limit(8)
            ->get()
            ->map(fn (MobilizationRequest $r) => [
                'id' => $r->id,
                'label' => '#'.$r->id.' · '.$r->destination,
                'meta' => trim(($r->requester?->first_name.' '.$r->requester?->last_name) ?: '').' · '.$r->status,
                'href' => '/app/secretaria/flujo',
            ]);

        return [
            'title' => 'Operación de Secretaría',
            'subtitle' => 'Cola de trabajo, flota y respaldos institucionales.',
            'period' => now()->format('Y-m'),
            'kpis' => [
                $this->kpi('pendientes', 'Por autorizar', $pendingAuth, 'warn', '/app/secretaria/autorizar', 'Bandeja de Secretaría'),
                $this->kpi('asignar', 'Por asignar unidad', $pendingAssign, $pendingAssign ? 'warn' : 'ok', '/app/secretaria/asignar', 'Conductor y vehículo'),
                $this->kpi('rectorado', 'En vicerrectorado', $pendingRectorate, 'info', '/app/secretaria/solicitudes', 'Viajes externos'),
                $this->kpi('aceptar', 'Conductor por aceptar', $acceptPending, $acceptPending ? 'warn' : 'ok', '/app/secretaria/reasignar'),
                $this->kpi('hoy', 'Viajes de hoy', $todayTrips, 'info', '/app/secretaria/agenda'),
                $this->kpi('aceite', 'Aceite vencido', $oilDue, $oilDue ? 'danger' : 'ok', '/app/secretaria/flota/vehiculos'),
                $this->kpi('novedades', 'Novedades abiertas', $openIssues, $openIssues ? 'danger' : 'ok', '/app/secretaria/taller'),
                $this->kpi('taller', 'OT abiertas / en taller', $openOrders + $inWorkshop, $openOrders ? 'warn' : 'ok', '/app/secretaria/taller'),
                $this->kpi('galones', 'Galones del mes', round($gallons, 1), 'info', '/app/secretaria/reportes'),
            ],
            'queue' => array_values(array_filter([
                $pendingAuth ? $this->queueItem('auth', 'Autorizar solicitudes', $pendingAuth, '/app/secretaria/autorizar', 'Revisar internas y externas pendientes.') : null,
                $pendingAssign ? $this->queueItem('assign', 'Asignar conductor y vehículo', $pendingAssign, '/app/secretaria/asignar', 'Ya tienen visto bueno y esperan unidad.') : null,
                ...
                $oilDue ? $this->queueItem('oil', 'Bloquear / enviar a cambio de aceite', $oilDue, '/app/secretaria/flota/vehiculos', 'Unidades que el sistema no dejará asignar.') : null,
                $openIssues ? $this->queueItem('issues', 'Seguir novedades de patio', $openIssues, '/app/secretaria/taller', 'Libro de novedades y órdenes de taller.') : null,
                $this->queueItem('pdf', 'Emitir respaldos PDF del periodo', 1, '/app/secretaria/reportes', 'Informe mensual F-007, aceite y novedades.'),
            ])),
            'charts' => [
            ...
            'recent' => $recent,
            'exports' => [
                ['label' => 'Informe mensual PAM-04-F-007', 'kind' => 'mensual', 'href' => '/app/secretaria/reportes'],
                ...
                ['label' => 'Documentos de solicitud', 'kind' => 'documentos', 'href' => '/app/secretaria/documentos'],
            ],
        ];
    }
    ...
    private function vicerrector(User $user): array
    {
        ...
        return [
            ...
            'subtitle' => 'Solo movilizaciones que requieren visto bueno de Vicerrectorado.',
            'period' => now()->format('Y-m'),
            'kpis' => [
                $this->kpi('pendientes', 'Por aprobar', $pending, $pending ? 'warn' : 'ok', '/app/vicerrector/pendientes'),
            ...
            'queue' => array_values(array_filter([
                ...
                $this->queueItem('pdf', 'Descargar informe mensual', 1, '/app/vicerrector/reportes', 'Respaldo PAM-04-F-007.'),
            ])),
            'charts' => [$this->chart('por_estado', 'Externas por estado', $byStatus)],
            'recent' => MobilizationRequest::where('mobilization_type', 'externa')->orderByDesc('id')->limit(6)->get()
                ->map(fn ($r) => [
                    'id' => $r->id,
                    'label' => '#'.$r->id.' · '.$r->destination,
                    'meta' => $r->status,
    ...
    private function facultad(User $user): array
    {
        ...
        return [
            ...
            'subtitle' => $user->faculty_institution ?: 'Solicitudes de su unidad académica.',
            'period' => now()->format('Y-m'),
            'kpis' => [
                $this->kpi('total', 'Solicitudes de la unidad', $total, 'info', '/app/facultad/solicitudes'),
                $this->kpi('pendientes', 'En trámite', $pending, $pending ? 'warn' : 'ok', '/app/facultad/seguimiento'),
            ],
            'queue' => [
                $this->queueItem('new', 'Registrar nueva salida', 1, '/app/facultad/solicitar', 'Complete ocupantes, actividad y N.º de comunicación.'),
                $this->queueItem('pdf', 'Emitir orden / hoja de ruta', 1, '/app/facultad/documentos', 'PDF de respaldo físico.'),
            ],
            'charts' => [$this->chart('por_estado', 'Estado de la facultad', $byStatus)],
            'recent' => (clone $base)->orderByDesc('id')->limit(6)->get()->map(fn ($r) => [
                'id' => $r->id,
                'label' => '#'.$r->id.' · '.$r->destination,
                'meta' => $r->status,
    ...
    private function docente(User $user): array
    {
        ...
        $total = (clone $base)->count();
        $pending = (clone $base)->whereIn('status', ['pendiente_secretaria', 'pendiente_rectorado'])->count();
        $approved = (clone $base)->whereIn('status', ['aprobada', 'autorizada_secretaria', 'aprobado_rectorado'])->count();
        $byStatus = (clone $base)->select('status', DB::raw('count(*) as total'))->groupBy('status')->pluck('total', 'status')->all();

        return [
            'title' => 'Mis movilizaciones',
            'subtitle' => 'Seguimiento de solicitudes, participantes y documentos.',
            'period' => now()->format('Y-m'),
            'kpis' => [
                $this->kpi('total', 'Solicitudes', $total, 'info', '/app/docente/historial'),
            ...
            'queue' => [
                $this->queueItem('new', 'Nueva solicitud', 1, '/app/docente/solicitar', 'Datos del formato PST-01-F-003.'),
                $this->queueItem('pdf', 'Descargar orden de movilización', 1, '/app/docente/documentos', 'Respaldo PDF para firmas.'),
            ],
            'charts' => [$this->chart('por_estado', 'Mis solicitudes', $byStatus)],
            'recent' => (clone $base)->orderByDesc('id')->limit(6)->get()->map(fn ($r) => [
                'id' => $r->id,
                'label' => '#'.$r->id.' · '.$r->destination,
                'meta' => $r->status,
